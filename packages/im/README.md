...
**剩下的那部分是布局，不是标记。** 每个流式 delta 都会改变最后一行的高度，而滚动跟随器紧接着读 `scrollHeight`，于是整列重新布局。所以行上带 `content-visibility: auto` 和 `contain-intrinsic-size: auto 8rem`：浏览器跳过屏幕外行的布局与绘制，但把它们留在 DOM 里——查找（Ctrl+F）、文本选择、屏幕阅读器都还能看到整段对话，而这正是虚拟滚动会放弃的东西。
