export const hsv2hsl = (h: number, s: number, v: number): { h: number; s: number; l: number } => {
...
export const hsl2hsv = (h: number, s: number, l: number): { h: number; s: number; v: number } => {
  s *= (l < 50 ? l : 100 - l) / 100;
...
export const rgba2rgb = (r: number, g: number, b: number, a: number): { r: number; g: number; b: number } => {
  a = a / 100;
