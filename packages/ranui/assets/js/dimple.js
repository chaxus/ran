...
dimple.axis = function (chart, position, categoryFields, measure, timeField, autoRotateLabel) {
    ...
    if (this.tickFormat !== null && this.tickFormat !== undefined) {
      ...
      returnFormat = function (n) {
        var l = Math.floor(Math.abs(n), 0).toString().length,
          ...
          d =
            Math.round((n / Math.pow(1000, c)) * 10)
              .toString()
              .slice(-1) === '0'
              ? 0
              : 1;
  ...
  this._update = function (refactor) {
    ...
    if (
      (refactor === null || refactor === undefined || refactor === false) &&
      !this._hasTimeField() &&
      this._scale !== null &&
      this._scale.ticks !== null &&
      this._scale.ticks !== undefined &&
      this._scale.ticks(tickCount).length > 0 &&
      ...
      if (runningY > self._heightPixels()) {
        ...
        d3.select(this)
          ...
          .attr(
            'x',
            self.horizontalAlign === 'left'
              ? self._xPixels() + runningX
              : self._xPixels() + (self._widthPixels() - runningX - maxWidth),
          )
...
dimple.series = function (
  ...
  this._axisBounds = function (position) {
    ...
    if (primaryAxis.showPercent) {
      ...
      secondaryAxis.categoryFields.length === 0
    ) {
      aggData.forEach(function (d) {
    ...
    for (i = 0; i < areaData.length; i += 1) {
      ...
      if (allPoints[group] && allPoints[group].length > 0) {
        ...
        for (j = 0, k = 0; j < allPoints[group].length; j += 1) {
          ...
          if (
            allPoints[group][j] >= points[0][catCoord] &&
            allPoints[group][j] <= points[points.length - 1][catCoord]
          ) {
...
dimple._helpers = {
  ...
  cy: function (d, chart, series) {
    ...
    if (series.y.measure !== null && series.y.measure !== undefined) {
      ...
      series.y.categoryFields.length >= 2
    ) {
      returnCy =
        series.y._scale(d.cy) -
