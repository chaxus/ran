...
| `transition: all` (or `background` in a transition) on a themed surface — button/input/select flash white→dark on theme toggle                                                                       | The theme switch fades `background-color` from the light to the dark token over the transition. List interaction props explicitly (`border-color`, `box-shadow`, `color`, `opacity`, `transform`) and never include `background`/`all`. DESIGN.md §5.                                                                                                                                                                                                                            |
