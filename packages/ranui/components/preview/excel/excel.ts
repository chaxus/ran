...
import {getDarkColor, getLightColor} from '../pptx/colz';
...
export async function getData(src:Blob | ArrayBuffer | Response, options:RequestExcelOption):Promise<unknown> {
...
export function transferExcelToSpreadSheet(workbook, options){
    ...
    workbook.eachSheet((sheet) => {
        ...
        const sheetData = { name: sheet.name,styles : [], rows: {},cols:{}, merges:[],media:[] };
        // 收集合并单元格信息
        ...
        (sheet._rows || []).forEach((row,spreadSheetRowIndex) =>{
            ...
            (row._cells || []).forEach((cell, spreadSheetColIndex) =>{
                ...
                const mergeAddress = find(mergeAddressData, function(o) { return o.startAddress == cell._address; });
