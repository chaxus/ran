...
> 📄 自动生成的 Token/Part 清单见 [style-tokens-parts.md](./style-tokens-parts.md)，可通过 `pnpm doc:style` 刷新。
> 📄 面向使用者的精简公开 API 见 [style-tokens-public.md](./style-tokens-public.md)。
> ⚙️ 公开 API 过滤规则可在 [style-token-filter.json](./style-token-filter.json) 中配置（支持全局规则与按组件覆盖）。
...
3. 默认视觉：图标尺寸默认 `1em`，颜色默认 `currentColor`，天然跟随文本排版和主题色。
4. 无障碍策略：
   - 装饰性图标默认 `aria-hidden="true"`。
   - 语义图标通过 `aria-label` 暴露，宿主元素设置 `role="img"`。
5. 性能策略：注册后缓存 SVG 文本，避免重复解析。
6. 组件自治策略：高频组件可在模块内部注册自身依赖图标（如 `message` 注册 `info-circle-fill/check-circle-fill/warning-circle-fill/close-circle-fill`），减少业务接入负担。
7. 诊断策略：开发环境对未注册图标输出 `console.warn`，生产环境静默降级。
