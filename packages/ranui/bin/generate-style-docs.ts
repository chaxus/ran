...
// The public view, published to the docs site in every language it ships. Every component
// page links to its own anchor here for "which CSS variables can I set on this element" —
// the answer was previously only inside the repository, so a reader on the site had no way
// to reach it. The table bodies are CSS identifiers and so are identical everywhere; only
// the page chrome (`style-page-copy.ts`) is translated.
...
async function main(): Promise<void> {
  ...
  for (const dir of DOC_LOCALE_DIRS) {
    const copy = STYLE_PAGE_COPY[dir];
    await emit(
      sitePagePath(ROOT, dir, 'ranui', 'style-tokens.md'),
      [
        '---',
        `title: ${copy.title}`,
        ...
        `# ${copy.heading}`,
        '',
        ...copy.intro((link) => siteHref(dir, link)),
        '',
        body,
      ].join('\n'),
    );
