...
- **i18n `t()` now escapes literal braces via doubling** (`{{` → `{`, `}}` → `}`), matching the Rust/Python/.NET format-string convention, so a message can show a literal `{token}` (write `{{token}}`) while still interpolating real `{param}` placeholders. Escaping and interpolation run in one left-to-right pass and apply with or without params. A lone or spaced brace (`{ ... }`) is still passed through untouched, so CSS/JSON/code in a message stays safe. **Behavior change:** any existing message containing a literal `{{` or `}}` that was *not* meant as an escape will now collapse to a single brace.
