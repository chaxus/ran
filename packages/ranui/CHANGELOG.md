...
- **`<r-form>` no longer owns an internal `<form>` — you now nest your own `<form>` inside it.** The previous design built a `<form class="r-form">` in shadow DOM and slotted fields into it through a named slot (`r-form_content`); this never actually worked in a real browser, for either native controls or form-associated custom elements (a `<form>` hidden inside shadow DOM can never become the form owner of light-DOM children — form ownership resolves over the real DOM ancestor chain, which does not cross into shadow roots — verified directly: a plain slotted `<input>`'s `.form` was `null` and invisible to `FormData`). `<r-form>` is now a thin wrapper: it requires a real `<form>` as a direct/descendant child, gives it a default layout (still overridable via CSS variables or `sheet`, now targeting `::slotted(form)` instead of the removed `.r-form`/`r-form_content`), calls `preventDefault()` on its `submit` (a real `<form>` would otherwise navigate the page) and serializes it to `value`, and clears `value` on `reset`. **Breaking (pre-1.0 alpha):** update any `<r-form>` usage from `<r-form><r-input slot="r-form_content" .../></r-form>` to `<r-form><form><r-input .../></form></r-form>`; `sheet` selectors targeting `.r-form` should target `::slotted(form)` (or the new `--ran-form-*` CSS variables) instead.
