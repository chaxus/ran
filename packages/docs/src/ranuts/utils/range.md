...
`remap(value, a1, a2, b1, b2)` linearly maps `value` from `[a1, a2]` onto `[b1, b2]`, unclamped. `fit` is the clamped version — the shader `fit`: the same remap, then clamped into the output range.
...
Both ramp from `0` to `1` as `x` goes from `edge0` to `edge1`, clamped outside that range. `linearstep` is a straight line; `smoothstep` is the GLSL Hermite-smoothed curve (`3t² - 2t³`) — an ease-in-ease-out instead of a linear ramp, the usual choice for animation and shader fades.
