...
fix (`fixDuration`) has actually run. Patching the duration needs the full buffer, which
is only available once the underlying `MediaRecorder`'s `stop` event has fired. In
practice this only matters if you read `recorder.stop()`'s return value at the exact
instant it resolves versus a moment later; awaiting the next microtask, or reading
`recorder.blob` after the `dataavailable`/`stop` sequence settles, is the safe order.
