...
1. **It assumes a monospaced grid**: every character advances one cell (CJK, full-width) or
   ...
   CJK-dominant body text — **not** a substitute for real shaping on proportional Latin.
...
empty and the loop never advances, which is a hang rather than a bad layout.
:::
