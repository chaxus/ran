...
description: 'ranuts がエクスポートする全シンボル —— 8 個のエントリーポイント、計 454 件のエクスポートを、シグネチャと説明つきで。'
---
...
制約、規約といった全体像は、まず [CLAUDE.md](https://github.com/chaxus/ran/blob/main/packages/ranuts/CLAUDE.md) を読んでください。

そのシンボルを持つ**サブパス**から import してください。たとえば `import { debounce } from
'ranuts/utils'`。ルートの `ranuts` は utils + visual の面を再エクスポートします。

**454 件のエクスポート**、エントリーポイント 8 個。

## エントリーポイント

- [`ranuts/utils`](#ranuts-utils) — ブラウザ向けおよび汎用のユーティリティ · _ブラウザ + node_ · 339 件のエクスポート
- [`ranuts/sw`](#ranuts-sw) — Service Worker のキャッシュ戦略とプリキャッシュのプロトコル · _service worker のみ_ · 9 件のエクスポート
- [`ranuts/node`](#ranuts-node) — Node サーバー向けユーティリティ（fs / http / ws / ミドルウェア） · _node のみ_ · 26 件のエクスポート
- [`ranuts/visual`](#ranuts-visual) — 2D レンダリングエンジン（Canvas / WebGL / WebGPU） · _ブラウザのみ_ · 16 件のエクスポート
- [`ranuts/i18n`](#ranuts-i18n) — フレームワーク非依存の i18n エンジン（ranuts/utils からも再エクスポート） · _ブラウザ + node_ · 9 件のエクスポート
- [`ranuts/vnode`](#ranuts-vnode) — Snabbdom 風の仮想 DOM · _ブラウザ_ · 26 件のエクスポート
- [`ranuts/stream`](#ranuts-stream) — Server-Sent Events の解析と、ベンダー中立なモデルストリームの畳み込み · _ブラウザ + node_ · 20 件のエクスポート
- [`ranuts/conversation`](#ranuts-conversation) — 追記のみのイベントログを描画可能な会話ノードへ射影する · _ブラウザ + node_ · 9 件のエクスポート

## `ranuts/utils`

ブラウザ向けおよび汎用のユーティリティ · 実行環境：**ブラウザ + node** · ソース：`src/utils/index.ts`
...
Service Worker のキャッシュ戦略とプリキャッシュのプロトコル · 実行環境：**service worker のみ** · ソース：`src/sw/index.ts`
...
Node サーバー向けユーティリティ（fs / http / ws / ミドルウェア） · 実行環境：**node のみ** · ソース：`src/node/index.ts`
...
2D レンダリングエンジン（Canvas / WebGL / WebGPU） · 実行環境：**ブラウザのみ** · ソース：`src/utils/visual/index.ts`
...
フレームワーク非依存の i18n エンジン（ranuts/utils からも再エクスポート） · 実行環境：**ブラウザ + node** · ソース：`src/utils/i18n.ts`
...
Snabbdom 風の仮想 DOM · 実行環境：**ブラウザ** · ソース：`src/vnode/index.ts`
...
Server-Sent Events の解析と、ベンダー中立なモデルストリームの畳み込み · 実行環境：**ブラウザ + node** · ソース：`src/stream/index.ts`
...
追記のみのイベントログを描画可能な会話ノードへ射影する · 実行環境：**ブラウザ + node** · ソース：`src/conversation/index.ts`
