...
> **何时使用**：需要在丰富、色彩浓烈的内容之上叠一层半透明面板时，比如 hero 卡片、悬浮工具条、媒体浮层。`displace` 控制"液态"程度（0 为纯磨砂平面）。在不支持 `backdrop-filter` 的环境会优雅降级为普通半透明面。

## 演练场

拖动舞台里的玻璃、调节每个属性、复制对应代码。默认参数就是 iOS 磨砂材质的观感。
...
> 把 `<r-glass>` 放在彩色或复杂的内容之上，纯色背景上看不出效果。
