...
优先用 WebGL 渲染——同步创建、几乎所有浏览器都支持，所以 rim 自己的首次出现不会被拖慢——如果浏览器同时支持 WebGPU，会在后台悄悄升级过去（效果完全一样，像素级一致；这种单次 draw call 的负载在 WebGPU 下不会渲染得更快，升级纯粹是为了面向未来兼容，不是为了性能）。两种 GPU API 都不可用时（很老的浏览器、被禁用、SSR）会静默降级为普通的 CSS 镜面渐变——不存在"坏掉"的中间状态。
...
- **背景采样**：`<r-glass>` 通过 `backdrop-filter` 折射它背后的 DOM，因此在普通页面内容上直接可用——包括背后可选中的文字、正在播放的视频、可交互元素。把**背景本身**栅格化再用 WebGL/WebGPU 着色器折射，理论上"液态感"更强、且三大浏览器表现一致，但代价是牺牲同样这些交互性/可访问性，且包体积明显更重——这条路刻意没走。上面的 `rim` 是折中方案：一个完全不碰背景、只根据面板自身形状计算的 GPU 装饰层。
