...
> **适用场景**：需要让用户通过饱和度/色相/透明度面板与 HEX/RGB 输入选择颜色时，`<r-colorpicker>` 接受并输出标准 CSS 颜色字符串，并在 `change` 时上报每一种格式。
...
拖动调色板、移动滑块、编辑数值输入框，或设置 `value` 特性，都会触发颜色变化。
...
- **输入格式**：给 `value` 传任意 CSS 颜色字符串，比如 HEX、`rgb(...)` 或 `rgba(...)`；选择器会在内部做归一化。
