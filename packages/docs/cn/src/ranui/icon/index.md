...
有效的自带名称就是 `RanIconName` 联合类型 / `RAN_ICON_NAMES` 元组（见下）。**自定义**名称若从未注册，仍然**什么都不渲染**（一片空白），这只针对你自己的 SVG，见 [自定义图标](#custom-icons)。
...
完整清单见[样式令牌](/cn/src/ranui/style-tokens#icon)；该选哪个令牌见[设计系统](/cn/src/ranui/design-system/)。
