...
> **适用场景**：当你要展示 Agent 或任务实际做了什么——一条 shell 命令、一次文件修改、一次查询——并且希望由工具说明「这是什么」、由界面决定「长什么样」时。

一个返回 HTML 的工具，等于替 UI 选好了渲染器、主题和布局，而且是在**面向模型的结果**里做这件事——这恰恰是最不该掺入 UI 关切的地方。声明意图把两者分开：同一次调用在这里可以是终端块，在紧凑的记录视图里是一行，在编辑器里是一个跳转目标，而工具完全不需要知道这些界面的存在。
...
这次调用**本身就是**一条 shell 命令。`title` 是命令；`description` 与 `cwd` 渲染在输出上方。非零 `exitCode` 会被显示，零则不显示。
...
这次调用创建或修改了文件。每一项都渲染为带双栏行号的 unified 风格 hunk，由 [ranuts/utils](../../ranuts/utils/) 的 `diffLines` 计算。**`oldText` 为 null 表示文件正在被创建**——这正是调用期视图所拥有的信息，因为调用方无法读取尚不存在的旧内容。
...
- **无法识别的卡片降级，绝不抛异常。** 来自更新版本生产方的卡片种类，或是在存储中被损坏的值，都会以 `generic` 渲染并保留它的标题；畸形视图渲染为空。展示层绝不能让重放崩掉。
