...
> **适用场景**：需要一个实验性的刮刮卡效果，在画布上拖动擦除覆盖层以显露下方任意内容——基于 Pointer Events，鼠标、触摸、触控笔都能用。

> ⚠️ **实验性组件**：本组件仍在开发中——当成一个好玩的交互效果，而不是打磨完善的生产级组件。
...
该组件**不会派发任何自定义事件**——没有什么可供绑定监听器的东西。刮除交互完全由内部注册在画布上的 [Pointer Events](https://developer.mozilla.org/zh-CN/docs/Web/API/Pointer_events) 驱动，鼠标、触摸、触控笔共用同一套逻辑：
...
- `pointerup` / `pointercancel` —— 退出待刮除状态；一旦累计面积超过画布像素面积的 **35%**，整个覆盖层会被 `clearRect` 清空，完全显露下方内容（这个阈值故意设得比较宽松——"刮一点就自动刮完"是刮刮卡常见的交互习惯，不需要把整个覆盖层手动刮干净）。
...
- **手势被打断**：如果系统在拖动过程中抢走了指针捕获、却从来没触发 `pointerup`（在部分 Android WebView 上，系统的返回手势打断刮除时会这样），`lostpointercapture` 监听器也会把内部状态重置回去——不然组件会以为自己还在刮除中，下一次毫不相关的指针移动会悄悄继续画下去。
