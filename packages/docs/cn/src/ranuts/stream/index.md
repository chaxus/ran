...
它同时接受 `AsyncIterable<Uint8Array>` 和 `ReadableStream`，因此测试可以直接传入字节切片，无需网络。

## 一份可参考的映射实现

本仓库的 `packages/im` 就是一个真实使用者：一条 OpenAI 兼容的 SSE 路由、到 `StreamChunk` 的映射，以及一个持有快照而非自行拼接增量的视图。它的往返测试会把真实服务端产生的字节、按多种分块大小传给真实客户端，因此两侧的分帧无法悄悄跑偏。
