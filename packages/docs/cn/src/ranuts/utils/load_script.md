...
**内联** script 在 append 的那一刻同步求值，之后**不会**再派发 `load` 事件。只等 `onload` 的话这个 Promise 在真实浏览器里永远不会 settle —— 所以 `type: 'content'` 时，`append` 返回即视为完成。（jsdom **会**为内联脚本派发 load 事件，这也是单测看不出这个差异的原因。）
