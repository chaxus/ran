...
| `onEnd`          | 每次录音结束都会触发一次——无论是主动停止、超时还是出错                | `() => void`                                     | `-`    |
...
2. **`supported` 和 `active` 都是 getter**，每次访问都会重新求值，而不是在创建时就固定下来。这一点在 `createSpeechRecognizer()` 于 `window` 或带前缀的构造函数出现之前就被调用的场景下很重要（服务端渲染、hydrate 之前的模块级早期调用）：一旦真正的 API 出现，识别器会自动感知到，而不会永远卡在 `supported === false`。
