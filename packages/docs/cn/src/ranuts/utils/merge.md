...
虽然名字像，其实是另一个工具：从一份由 getter 组成的映射构建出一个**惰性求值、冻结**的导出对象，而不是拷贝普通值。每个 getter 最多执行一次（首次访问时），之后结果就会被缓存下来，用的正是 `ranuts/utils` 单独导出的那个 `once` 包装器。嵌套的普通对象会被递归合并并冻结；其他值（既不是 getter 也不是嵌套对象）会直接抛错。
...
1. **不是通用的合并函数。** 普通值合并用 `merge`；`mergeExports` 是用来构建"模块形状"的对象，其中一些属性计算代价较高，只应该在真正被读取时才执行。
