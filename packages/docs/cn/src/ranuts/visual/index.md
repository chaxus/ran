...
一个 PixiJS 风格的 2D 渲染引擎。构建由图形组成的场景树，并通过三种后端之一（Canvas2D、WebGL 或 WebGPU）在运行时渲染。
...
组节点，即场景树中的“组”概念。它持有子节点与变换状态，但自身不渲染任何内容；`Graphics` 等可绘制对象继承自它。用 `Container` 可以把需要一起平移 / 缩放 / 旋转的子树组合在一起。
