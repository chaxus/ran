...
۱. **برای دارایی‌های تغییرناپذیر و هش‌دار `cacheFirst`**: اسکریپت‌ها، استایل‌ها، قلم‌ها، وزن‌های مدل. **برای هرچه باید انتشار تازه را بی‌درنگ بازتاب دهد `networkFirst`**: ناوبری‌های HTML، یک مانیفست.
۲. **هیچ‌کدام از این دو راهبرد reject نمی‌کنند.** شکست شبکه در نبود چیزی در کش به یک ۴۰۸ حل می‌شود، پس `respondWith` هرگز خطا پرتاب نمی‌کند.
۳. **پاسخ پیش از خوانده‌شدن بدنه، به‌طور همگام کلون می‌شود.** اول منتظر `caches.open()` ماندن و بعد کلون کردن، همان اشکال کلاسیک است: تا آن موقع ممکن است بدنه از پیش به‌سوی صفحه در جریان باشد و `clone()` خطا بدهد.
۴. **`precache` خودتوان است و در سطح هر URL بخشنده**: یک ۴۰۴ در فهرست نباید نصب را از کار بیندازد.
۵. **دانلود کردن در خود SW دقیقاً نکته `servePrecache` است.** آن کار درون `event.waitUntil` پیچیده می‌شود، پس از ناوبری‌ها جان به در می‌برد؛ اما واکشیِ سمت صفحه همان لحظه که کاربر جای دیگری می‌رود لغو می‌شود و یک دارایی بزرگ در بازدید بعدی از صفر آغاز می‌کند.
