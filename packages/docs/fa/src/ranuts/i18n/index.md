...
2. **نوع از آرگومان نوع می‌آید، هرگز از داده.** `messages` در `NoInfer` پیچیده شده، پس زبان‌هایی با مجموعه‌کلیدهای متفاوت نمی‌توانند TypeScript را وادار کنند _اشتراک_ آن‌ها را استنتاج کند. وگرنه کلیدی که تنها زبان جایگزین تعریفش کرده در هر محل فراخوانی رد می‌شد و ترجمه‌ای ناتمام به‌جای بازگشت در زمان اجرا، بیلد را می‌شکست.
