...
۱. **ترتیب اجرا**: میان‌افزارها به ترتیب آرایه اجرا می‌شوند و پس از فراخوانی `next()` نوبت به بعدی می‌رسد.
۲. **پشتیبانی ناهمگام**: همهٔ میان‌افزارها باید ناهمگام باشند یا Promise برگردانند.
۳. **فراخوانی `next()`**: برای رفتن به میان‌افزار بعدی باید درون میان‌افزار `next()` را صدا بزنید.
۴. **فراخوانی چندباره**: `next()` را نمی‌توان چند بار صدا زد، وگرنه خطا پرتاب می‌شود.
۵. **انتقال بستر**: داده میان میان‌افزارها از راه شیء `context` رد و بدل می‌شود.
