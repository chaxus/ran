...
۱. **ساخت تصادفی**: هر بار فراخوانی، یک مقدار رنگ شانزده‌شانزدهیِ تصادفی می‌سازد.
۲. **شیء کامل**: یک شیء `Color` کامل با همهٔ ویژگی‌ها — hex، rgb، hsl و بقیه — برمی‌گرداند.
۳. **قالب رنگ**: مقدار ساخته‌شده نشانهٔ `#` را دارد و می‌توان آن را مستقیم در CSS به کار برد.
۴. **کاربرد**: معمولاً برای ساخت رنگ تصادفی، انتخابگر رنگ و مصورسازی داده به کار می‌رود.
