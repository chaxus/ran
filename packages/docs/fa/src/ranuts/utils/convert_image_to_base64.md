...
2. **گونهٔ فایل**: هر قالب تصویری که مرورگر بشناسد (JPEG، PNG، GIF، WebP و مانند آن).

3. **قالب داده**: `data`ی بازگشتی یک Data URL کامل است (`data:image/jpeg;base64,...`) و همان‌طور در ویژگی `src` تگ `img` جا می‌افتد.

4. **مدیریت خطا**: اگر تبدیل شکست بخورد Promise رد می‌شود؛ آن را بگیرید.

5. **کاربرد**: معمولاً برای پیش‌نمایش تصویر، پردازش پیش از بارگذاری و ذخیرهٔ محلی به کار می‌رود.
