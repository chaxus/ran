...
1. **هر سه خروجی سیم‌کشی شده‌اند**: `onload`، `onerror` و `onabort`. فراموش‌کردن `onabort` همان راه کلاسیک است که وقتی کاربر انتخابگر فایل را لغو می‌کند، یک پرامیس را تا ابد در انتظار می‌گذارد.
2. **با خطایی روشن reject می‌کند** جایی که `FileReader` وجود ندارد (Node و برخی بسترهای ورکر).
3. **هرگز روی فایلی با منشأ ناشناخته `new TextDecoder().decode()` نزنید**: این کار UTF-8 را فرض می‌گیرد و GBK/Big5 را به هم می‌ریزد. از `arrayBufferToString` استفاده کنید که اول تشخیص می‌دهد.
