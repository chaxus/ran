...
۱. **تابع کاری‌شده**: تابعی برمی‌گرداند که زمان تأخیر را می‌پذیرد و با سبک تابعی جور درمی‌آید.
۲. **پشتیبانی از ناهمگام**: توابع ناهمگام را هم می‌پذیرد و تا پایان اجرا صبر می‌کند.
۳. **مدیریت خطا**: اگر اجرای تابع شکست بخورد، Promise رد می‌شود.
۴. **کاربرد**: معمولاً برای اجرای تأخیری، کارهای زمان‌بندی‌شده و سنجش شبکه به کار می‌رود.
