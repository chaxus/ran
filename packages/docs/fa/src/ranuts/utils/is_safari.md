...
۱. **شیوه تشخیص**: بررسی می‌کند که 'Apple' در `navigator.vendor` هست یا نه.
۲. **کنار گذاشتن مرورگرهای دیگر**: کروم iOS (CriOS) و فایرفاکس iOS (FxiOS) را بیرون می‌گذارد.
۳. **محیط سمت سرور**: در محیط سرور (بدون شیء `navigator`) مقدار `undefined` برمی‌گرداند.
۴. **مقدار بازگشتی**: در مرورگر `boolean`، در سرور `undefined`؛ در برخی حالت‌ها ممکن است رشته برگرداند.
