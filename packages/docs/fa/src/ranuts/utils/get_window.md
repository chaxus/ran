...
۱. **سازگاری با مرورگرها**: از `window.innerWidth` و `window.innerHeight` استفاده می‌کند که در همهٔ مرورگرهای امروزی هست.

۲. **امن در سمت سرور**: در محیط‌های سرور (بدون شیء `window`) مقدار `{ width: 0, height: 0 }` برمی‌گرداند و خطایی پرتاب نمی‌کند.

۳. **همان لحظه**: اندازه را در لحظهٔ فراخوانی برمی‌گرداند؛ اگر پنجره تغییر کرد، دوباره صدایش بزنید.

۴. **کاربرد**: معمولاً در چیدمان‌های واکنش‌گرا، پرس‌وجوهای رسانه‌ای و پایش اندازهٔ پنجره به کار می‌رود.
