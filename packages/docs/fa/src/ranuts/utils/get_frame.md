...
1. **یکا**: نرخ فریم در هر میلی‌ثانیه برمی‌گردد؛ برای نرخ در هر ثانیه (FPS) در ۱۰۰۰ ضرب کنید.
2. **شیوهٔ نمونه‌برداری**: با `requestAnimationFrame` نمونه می‌گیرد و میانگین فاصلهٔ چند فریم را حساب می‌کند.
3. **ناهمگام**: Promise برمی‌گرداند، پس با `await` یا `.then()` رسیدگی کنید.
4. **کاربرد**: معمولاً برای پایش کارایی، وارسی پویانمایی و رصد نرخ فریم بازی به کار می‌رود.
