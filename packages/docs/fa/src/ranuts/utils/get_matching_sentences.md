...
۱. **مرز جمله**: با نقطهٔ ایدئوگرافیک (。)، نقطه (.)، خط تازه (\n)، علامت تعجب (！) و پرسش (?، ？) مرز جمله را می‌شناسد.
۲. **حذف تکرار**: وقتی جمله‌ها روی هم می‌افتند، تنها بلندترین می‌ماند.
۳. **بی‌توجه به بزرگی حروف**: جست‌وجو بزرگی و کوچکی حروف را نادیده می‌گیرد.
۴. **کاربرد**: معمولاً برای برجسته‌سازی نتیجهٔ جست‌وجو، خلاصه‌سازی متن و نمایش نتایج به کار می‌رود.
