...
۱. **زنجیره‌سازی**: همهٔ متدها نمونهٔ `Chain` را برمی‌گردانند، پس می‌توان آن‌ها را به هم زنجیر کرد.
۲. **پشتیبانی از SVG**: تگ‌های SVG را خودش می‌شناسد و با فضای‌نام درست می‌سازد.
۳. **مدیریت رویداد**: در درون، نگاشت شنونده‌ها را نگه می‌دارد و همین اداره و برداشتنشان را آسان می‌کند.
۴. **کاربرد**: معمولاً برای ساختن پویای ساختار DOM و سرهم کردن مؤلفه‌های رابط کاربری به کار می‌رود.
