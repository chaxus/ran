...
1. **محیط‌های پشتیبانی‌شده**:

- مرورگر: از `performance.now()` استفاده می‌کند
- Node.js: از `process.hrtime()` استفاده می‌کند
- سایر محیط‌ها: به `Date.now()` بازمی‌گردد

2. **دقت**: `performance.now()` و `process.hrtime()` تا میکروثانیه دقت دارند و از `Date.now()` دقیق‌ترند.

3. **زمان نسبی**: برچسب زمانی بازگشتی نسبی است؛ برای سنجش اختلاف زمان مناسب است، نه برای استفاده به‌عنوان زمان مطلق.

4. **یکا**: مقدار بازگشتی بر حسب میلی‌ثانیه است.
