...
7. **`serveWorker` خطاهای پرتاب‌شدهٔ همگام را هم می‌گیرد.** پرتاب همگام درون `onmessage` به رسیدگی‌کنندهٔ خطای کارگر می‌گریزد و در آن مسیر هیچ `operationId` همراه نیست؛ آنگاه کارخواه ناچار است به‌جای همان یکی که واقعاً شکست، _همهٔ_ درخواست‌های در جریان را ناکام کند.
