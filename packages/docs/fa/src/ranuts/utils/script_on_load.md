...
1. **تشخیص خودکار نوع**: از روی پسوند نشانی (`.css`) خودش سبک را از اسکریپت جدا می‌کند.
2. **بارگذاری موازی**: همهٔ منابع هم‌زمان بارگذاری می‌شوند و تنها پس از پایان همه برآورده می‌شود.
3. **جای درج**: به‌طور پیش‌فرض عنصر `body`، ولی می‌توان والد دیگری تعیین کرد.
4. **Promise و فراخوان**: هر دو پشتیبانی می‌شوند و می‌توان با هم به کار برد.
