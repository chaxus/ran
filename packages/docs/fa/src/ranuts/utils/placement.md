...
۱. **مختصات سرتاسر نسبت به دیدگاه‌اند**، در همان فضای `anchor`. اگر تابلو را با `position: absolute` نسبت به سند می‌نشانید، هنگام نوشتن سبک، `scrollX` و `scrollY` را خودتان بیفزایید (نمونهٔ بالا را ببینید).
...
۳. **اگر تابلو از خودِ کرانه بزرگ‌تر باشد، لغزاندن انجام نمی‌شود**: مهار کردنش تنها آن را از سوی دیگر بیشتر از پرده بیرون می‌راند.
۴. `r-popover` و `r-select` در `ranui` در درون از آن بهره می‌برند تا فهرست بازشویی که به `body` منتقل شده روی پرده بماند.
