...
1. **تطبیق با عبارت باقاعده**: کوکی را با عبارت باقاعده پیدا می‌کند، پس فاصله‌های پیش یا پس از نام مشکلی ایجاد نمی‌کنند.
2. **امن در سمت سرور**: در محیط‌های سرور (بدون شیء `window`) رشتهٔ خالی برمی‌گرداند و خطایی پرتاب نمی‌کند.
3. **تفاوت با getCookie**: کارشان یکی است، اما `getCookieByName` از عبارت باقاعده و `getCookie` از تکه‌کردن رشته استفاده می‌کند.
4. **مقدار بازگشتی**: وقتی کوکی وجود ندارد رشتهٔ خالی برمی‌گردد، نه `null` و نه `undefined`.
