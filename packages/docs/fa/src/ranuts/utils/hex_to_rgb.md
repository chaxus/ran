...
۱. **قالب پذیرفته‌شده**: مقدارهای رنگ شانزده‌شانزدهیِ شش‌رقمی (مثلاً `#ff0000` یا `ff0000`).
۲. **مقدار بازگشتی**: در صورت موفقیت آرایهٔ `[r, g, b]` و در صورت شکست `null`.
۳. **بزرگی و کوچکی حروف**: `#FF0000` و `#ff0000` یکسان‌اند.
۴. **کاربرد**: معمولاً برای تبدیل و پردازش رنگ و کار با رنگ‌های CSS به کار می‌رود.
