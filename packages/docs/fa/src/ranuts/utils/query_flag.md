...
1. **رایج آن است که پرچم را بی‌مقدار بنویسند.** `?embed` مقداری ندارد، پس `getQuery(url).embed` برابر `''` است (که نادرست ارزیابی می‌شود) و وارسی سادهٔ درستی، خاموشانه از کنار رایج‌ترین شکل می‌گذرد. `queryFlag` برای همین هست.

2. **`?k=false` نادرست است.** نفیِ صریح ارج نهاده می‌شود، نه اینکه «هست، پس روشن است» خوانده شود.

3. **`isInIframe` محافظت شده است.** خواندن `window.parent` در برخی موتورها میان مبدأهای گوناگون خطا می‌دهد؛ والدی که خوانده نشود، جاسازی‌شده به شمار می‌آید، چون معنایش دقیقاً همین است.

4. **هر دو در SSR امن‌اند.** بی `window`، مقدار `isInIframe` برابر `false` است و `queryFlag` نیز `false` مگر آنکه `url` بدهید؛ پس با دادن نشانی، هر دو در اسکریپت‌های زمان ساخت هم کار می‌کنند.
