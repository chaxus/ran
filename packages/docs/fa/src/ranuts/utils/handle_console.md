...
۱. **رفتار اصلی دست‌نخورده می‌ماند.** پاسخ‌ها از میان می‌گذرند، خطاها دوباره پرتاب می‌شوند و کنسول همچنان چاپ می‌کند.
۲. **`restore` در `replaceOld` تنها وصلهٔ خودش را برمی‌دارد.** اگر پس از آن لایهٔ دیگری روی آن وصله زده باشد، برگرداندن کورکورانه آن لایه را خاموشانه از جا برمی‌کند، پس در آن حالت از این کار سر باز می‌زند.
۳. **`handleXhrHook` به نمونهٔ اولیه وصله می‌زند**، پس بر همهٔ نمونه‌ها اثر می‌گذارد؛ شنونده‌هایش با `{ once: true }` ثبت می‌شوند تا شیء XHRِ بازاستفاده‌شده آن‌ها را روی هم انبار نکند.
۴. **خروجی کنسول را به پشتیبانی که خودش در کنسول می‌نویسد گزارش نکنید**: آن قلاب با همان فراخوانی‌ای شلیک می‌کند که خودش پدید آورده است. (به همین سبب کانال `console` در `Monitor` به‌طور پیش‌فرض خاموش است.)
