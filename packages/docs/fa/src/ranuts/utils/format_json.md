...
۱. **رشتهٔ ورودی بازتجزیه می‌شود، نه اینکه همان‌طور پس داده شود.** اینچنین درستی‌اش وارسی و چیدمانش یکدست می‌شود، به‌جای اعتماد به فاصله‌گذاری فراخواننده.
۲. **خطاها گزارش می‌شوند، هرگز پرتاب نمی‌شوند.** JSON نامعتبر، ساختارهای چرخه‌ای و مقدارهایی که `JSON.stringify` نمی‌تواند بنماید، همگی `''` برمی‌گردانند و `onError` را صدا می‌زنند.
