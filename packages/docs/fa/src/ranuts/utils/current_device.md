...
1. **ترتیب تشخیص**: به این ترتیب بررسی می‌کند:
...
2. **رندر سمت سرور**: در محیط‌های سرور (بدون شیء `window`) مقدار `'pc'` را برمی‌گرداند.

3. **روش تشخیص**: از روی رشتهٔ User Agent.

4. **مقدار بازگشتی**: از نوع شمارشی است و تنها یکی از `'ipad'`، `'android'`، `'iphone'` یا `'pc'` می‌تواند باشد.
