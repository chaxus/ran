...
1. **اجرای همگام**: متد `call` همهٔ فراخوان‌ها را همگام اجرا می‌کند.
2. **پشتیبانی ناهمگام**: متد `callSync` فراخوان‌های ناهمگام را می‌پذیرد و تا پایان همه صبر می‌کند.
3. **مدیریت رویداد**: در درون از `Map` و `Set` برای نگهداری رویدادها و فراخوان‌ها استفاده می‌کند.
4. **کاربرد**: معمولاً در سامانه‌های رویداد، سامانه‌های افزونه و میان‌افزارها به کار می‌رود.
