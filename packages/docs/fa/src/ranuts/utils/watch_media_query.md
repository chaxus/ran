...
۱. **کال‌بک یک بار و همگام** با مقدار کنونی صدا زده می‌شود، پس هرگز لازم نیست وضعیت آغازین را جداگانه بخوانید.
۲. **همیشه اشتراک را لغو کنید.** شنونده `MediaQueryList`ی که آزاد نشود، بستار (و هر DOMی که گرفته) را زنده نگه می‌دارد.
۳. **سافاری قدیمی هم دیده شده است.** `addEventListener` روی `MediaQueryList` تازه در سافاری ۱۴ آمد؛ به‌عنوان جایگزین از `addListener`/`removeListener` استفاده می‌شود.
