...
1. **بررسی تکرار**: اگر عنصر آن کلاس را داشته باشد، دوباره افزوده نمی‌شود.
2. **ایمنی سمت سرور**: در محیط سرور (بدون شیء `document`) بی‌صدا و بدون پرتاب خطا رفتار می‌کند.
3. **از classList استفاده می‌کند**: از `classList.add()` امروزی بهره می‌برد که از دستکاری مستقیم `className` ایمن‌تر است.
