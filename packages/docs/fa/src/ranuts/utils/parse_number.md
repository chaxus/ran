...
1. **فقط بخش عددی را بدهید.** `第三章` مقدار `null` می‌دهد: نخست `三` را با الگوی خودتان بیرون بکشید و همان را تجزیه کنید.
2. **`十` که چیزی پیش از آن نباشد یعنی ۱**، پس `十五` می‌شود ۱۵ نه ۵.
3. **`parseEnglishNumber` نخست رقم، سپس واژه و سپس عدد رومی را می‌آزماید.** `twenty-one` و بالاتر پوشش داده نشده‌اند؛ اگر لازم دارید جدول واژه‌ها را گسترش دهید.
