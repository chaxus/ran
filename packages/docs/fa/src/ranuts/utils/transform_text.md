...
1. **تشخیص خودکار**: برای یافتن نوع کدگذاری از `jschardet` استفاده می‌کند.
2. **تنها `ArrayBuffer`**: فعلاً فقط `ArrayBuffer` پشتیبانی می‌شود و دادن رشته هشدار می‌دهد.
3. **شرط بازگشت نتیجه**: تنها وقتی کدگذاری تشخیص داده شود و رمزگشایی موفق باشد نتیجه برمی‌گردد، وگرنه `undefined`.
4. **کاربرد**: معمولاً در خواندن فایل، رمزگشایی متن و تبدیل کدگذاری به کار می‌رود.
