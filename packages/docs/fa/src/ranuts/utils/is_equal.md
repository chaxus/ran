...
1. **سنجش ژرف**: همهٔ ویژگی‌های شیءها و آرایه‌ها را بازگشتی می‌سنجد.
2. **ارجاع چرخه‌ای**: درست رسیدگی می‌شود.
3. **وارسی نوع**: نوع مقدارها را هم می‌بیند و اگر فرق داشته باشند `false` می‌دهد.
4. **کارایی**: برای شیءها یا آرایه‌های بزرگ، سنجش ژرف می‌تواند کند باشد.
