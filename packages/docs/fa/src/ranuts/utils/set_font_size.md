...
1. **برای iPad پایهٔ دیگری خودبه‌خود برگزیده می‌شود.** وقتی `currentDevice()` از iPad خبر دهد، به‌جای `designWidth`ای که داده‌اید، پهنای طرح و نسبت ابعاد به `768` / `1024:768` می‌رود. این تابع به‌طور پیش‌فرض طرح گوشی را فرض می‌گیرد و تنها همین یک استثنای پرتکرار را جبران می‌کند.
