...
> **이럴 때 쓰세요.** 페이지의 주요 영역을 접근성 있는 2단계 제목과 선택적 부제로 표시해야 할 때. `<r-section>`이 제목 줄과 본문 표면을 제공합니다.
...
| 프로퍼티   | 타입     | 기본값 | 설명                                        |
| ---------- | -------- | ------ | ------------------------------------------- |
| `heading`  | `string` | `''`   | 구역 제목. ARIA 2단계 제목으로 렌더링됩니다 |
| `subtitle` | `string` | `''`   | 제목 아래에 붙는 보조 한 줄                 |
| `sheet`    | `string` | `''`   | 구역의 섀도 DOM에 주입할 CSS                |

`heading`과 `subtitle`이 모두 비어 있으면 제목 줄 전체가 숨겨집니다.

### 제목 `heading`

구역 제목이며 ARIA 2단계 제목(`role="heading"`, `aria-level="2"`)으로 렌더링됩니다. 비어 있으면 숨겨집니다.
...
구역의 섀도 DOM에 주입하는 CSS로, 다른 모든 ranui 컴포넌트와 같은 `sheet` 관례를 따릅니다.
...
- **접근성**: 제목은 ARIA 2단계 제목으로 노출되어 문서 개요에 참여합니다. 의미 있는 제목을 쓰세요.
