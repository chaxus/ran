...
HTML을 돌려주는 도구는 UI를 대신해 렌더러와 테마와 레이아웃을 골라 버린 것이고, 하필 UI의 관심사가
...
0이 아닌 `exitCode`는 드러내고, 0은 드러내지 않습니다.
...
호출이 파일을 만들거나 고치는 경우입니다. 각 항목은 양쪽 여백을 갖춘 unified 형식의 hunk로 그려지며,
[ranuts/utils](../../ranuts/utils/)의 `diffLines`가 계산합니다. **`oldText`가 null이면 파일을 새로
...
- **뷰는 호출 인자의 순수 함수입니다**(결과 뷰라면 결과까지 포함해서). I/O도, 시계도, 세션 상태도
...
`<r-tool-card>`는 자체 **CSS 커스텀 프로퍼티 24개**와 테마에서 읽어 오는 의미 토큰을 공개합니다.
