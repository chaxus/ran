...
견본을 클릭하거나(또는 포커스를 준 뒤 Enter/Space를 눌러) 패널을 엽니다. 색상과 투명도 슬라이더는 키보드로도 조작됩니다. 화살표 키는 1씩, Shift+화살표는 10씩 움직이고, Home/End는 양 끝으로 뜁니다.
...
현재 색을 CSS 색 문자열로 나타냅니다. 입력으로는 HEX(`#1677FF`, `#fff`), `rgb(...)`, `rgba(...)`를 받습니다. 읽어 낼 때의 표준형은 완전히 불투명하면 6자리 HEX 문자열, 투명도가 1보다 작으면 `rgba(...)` 문자열입니다.
