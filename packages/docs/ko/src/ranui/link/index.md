...
내부 `href`를 클릭하면 링크가 그 경로를 활성 ranui 라우터에 넘깁니다(`push`, `replace` 어트리뷰트가 있으면 `replace`). 외부 링크(`https://`, `//`, `mailto:`, `tel:`)와 보조 키를 누른 클릭(가운데 버튼, Ctrl/Cmd/Shift/Alt)은 평소대로 브라우저로 넘어갑니다. 등록된 라우터가 없으면 대신 버블링되고 composed된 `ran-navigate` 이벤트를 디스패치합니다.
...
| 프로퍼티  | 타입      | 기본값  | 설명                                                                              |
| --------- | --------- | ------- | --------------------------------------------------------------------------------- |
| `href`    | `string`  | `''`    | 이동 대상. 내부 경로는 앱 안에서 처리되고 외부 URL은 평소대로 이동합니다          |
| `replace` | `boolean` | `false` | 있으면 앱 안의 이동이 현재 히스토리 항목을 대체합니다(읽기 전용, 어트리뷰트 반영) |
| `sheet`   | `string`  | `''`    | 링크의 섀도 DOM에 주입할 CSS                                                      |

### 이동 대상 `href`

내부 경로는 앱 안에서 처리되고, 절대 URL과 `mailto:` / `tel:` 링크는 평소대로 이동합니다.
...
링크의 섀도 DOM에 주입하는 CSS로, 다른 모든 ranui 컴포넌트와 같은 `sheet` 관례를 따릅니다. 클릭 대상인 `<a>`가 섀도 루트 안에 있으므로, 호스트를 버튼이나 카드처럼 보이게 하려면 `sheet`로 박스 모델(`display`, `padding`, `width`)을 지정하세요.
...
| 이벤트         | detail                               | 발생 시점                                                                      |
| -------------- | ------------------------------------ | ------------------------------------------------------------------------------ |
| `ran-navigate` | `{ path: string, replace: boolean }` | 내부 링크를 클릭했는데 활성 ranui 라우터가 없을 때. 버블링되고 composed입니다. |
...
- **외부 링크**: 절대 URL과 `mailto:` / `tel:`는 브라우저로 넘어갑니다. 추가 설정은 필요 없습니다.
