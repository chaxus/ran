---
description: 'ranui의 Checkbox(<r-checkbox>)는 켬/끔 선택 하나를 토글합니다. 레이블을 붙일 수 있고 네이티브 폼을 지원합니다.'
---
...
| `sheet`    | `string`  | `''`      | 모습을 바꾸려고 컴포넌트의 섀도 DOM에 주입할 CSS   |
...
`sheet` 어트리뷰트는 섀도 DOM에 CSS를 주입해, 내부 요소를 클래스 이름으로 겨냥할 수 있게 합니다.
...
호스트 자신이 접근성 있는 체크박스 의미를 갖습니다: `role="checkbox"`, `aria-checked`, `aria-disabled`, 그리고 키보드 조작(Space나 Enter로 토글).
...
`<r-checkbox>`는 자체 **CSS 커스텀 프로퍼티 32개**와 테마에서 읽어 오는 의미 토큰을 공개합니다.
...
- **checked와 value**: 불리언 상태에는 `checked`를 쓰고, 폼 데이터를 모을 때는 `value`(`'true'` / `'false'`)를 읽으세요.
- **비활성 상태**: 그 선택을 쓸 수 없을 때 `disabled`를 쓰세요.
- **`change`를 구독하세요**: DOM을 다시 질의하지 말고 `event.detail.checked`를 읽으세요.
