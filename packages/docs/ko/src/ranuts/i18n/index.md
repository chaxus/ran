...
2. **타입은 타입 인자에서 오지, 데이터에서 오지 않습니다.** `messages`는 `NoInfer`로 감싸여 있어서, 키 집합이 다른 로케일들로부터 TypeScript가 그 **교집합**을 추론해 버리는 일이 없습니다. 그러지 않으면 대체 로케일만 정의한 키가 모든 호출부에서 거부되고, 번역이 덜 되었을 뿐인데 빌드가 깨집니다. 실행 중에 대체하면 그만인 일인데도요.
