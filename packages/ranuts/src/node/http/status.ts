...
function createMessageToStatusCodeMap(codes: Codes) {
  ...
  Object.keys(codes).forEach((code) => {
    ...
    const message:string = codes[code as unknown as keyof Codes]
    const status = Number(code)

    // populate map
    map[message.toString().toLowerCase()] = status
  })
...
function getStatusMessage(code:keyof Codes) {
...
function getStatus(code: keyof Codes | string) {
  ...
  if (!isNaN(n) && status.codes.includes(n)) {
    // return message
    return getStatusMessage(n as keyof Codes)
  }
  // return code
